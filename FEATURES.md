...
| 11 | Obsidian-Inbox lesen/schreiben `[ACTION:INBOX_READ/WRITE]` | [actions.py](actions.py) `spec.execute` (RFC-0001) | ✅ unverändert |
...
| 24 | Klare Orb-Zustände: idle/listening/thinking/speaking/**muted**/**error** | [frontend/voice.js](frontend/voice.js) `presentation` (reiner Reducer, RFC-0006 Phase 4J — `setOrbState` entfernt), [frontend/main.js](frontend/main.js) `renderVoice`, [frontend/style.css](frontend/style.css) | ✅ neu |
...
| 33 | Asynchroner Startup: Server nimmt sofort Verbindungen an, Wetter/Tasks/Vault laden im Hintergrund über die `context.refresh`-Capability | [runtime.py](runtime.py) `refresh_context` (Startup-Task), [capability/](capability/) `context.refresh` | ✅ neu |
...
| 43 | Sitzungszusammenfassung `[ACTION:SESSION_SUMMARY]` („Was haben wir heute gemacht?"), speicherbar via INBOX_WRITE | [actions.py](actions.py) `spec.execute` (RFC-0001) | ✅ neu |
| 44 | Bestätigungs-Mechanismus für destruktive Aktionen: `CONFIRM_ACTIONS` wird aus dem `destructive`-Effekt des Capability-Vertrags **abgeleitet** (kein gespeichertes `risk` mehr; aktuell nur `MEMORY_FORGET`) → mündliche Ja/Nein-Rückfrage vor Ausführung | [actions.py](actions.py) `CONFIRM_ACTIONS`, [conversation/](conversation/) Bestätigungs-Flow | ✅ vertieft (Phase 5C) |
| 45 | Mini-Aktionshistorie auch im Panel-Modus (kompakte Leiste, letzte ~3 Einträge) | [frontend/style.css](frontend/style.css) | ✅ neu |
| 46 | Zentrale Action-Registry: Label, Payload-Regel, Timeout, Summary-Prompt, **Ausfuehrung (`execute`) und Prompt-Selbstbeschreibung (`describe`)** pro Aktion in einem `ActionSpec` — der System-Prompt wird daraus erzeugt; `risk` ist eine abgeleitete Property (kein gespeichertes Feld). Seit Phase 5C laufen alle 22 Actions über den Capability-Coordinator (kein `execute_action`-Fallback) | [actions.py](actions.py) `REGISTRY`, [capability/](capability/) | ✅ vertieft (RFC-0001/RFC-0007) |
...
| 51 | Vault-Kontext-Broker `[ACTION:PROJECT_CONTEXT] frage`: lokale, token-sparsame Vault-Suche (Ranking nach Dateiname/Überschrift/Ordner/Text + Recency/Tags), nur kurze Ausschnitte ans LLM, Secret-Dateien/-Zeilen ausgenommen | [memory.py](memory.py) `get_project_context_sync`, [actions.py](actions.py) `spec.execute` (RFC-0001) | ✅ neu |
